...
#define DEFAULT_OMIT_ENDLIST FALSE
...
gst_hls_sink_class_init (GstHlsSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_OMIT_ENDLIST,
      g_param_spec_boolean ("omit-endlist", "Omit Endlist",
          "Do not append endlist tag",
          DEFAULT_OMIT_ENDLIST, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
gst_hls_sink_init (GstHlsSink * sink)
{
  ...
  sink->omit_endlist = DEFAULT_OMIT_ENDLIST;
...
static void
gst_hls_recover_playlist (GstHlsSink * sink)
{
  ...
  if (g_file_test (sink->playlist_location, G_FILE_TEST_EXISTS)) {
    ...
    for (walk = m3u8->files; walk; walk = walk->next) {
      ...
      gst_m3u8_playlist_add_entry (sink->playlist, file->uri + 2, file->title,
          GST_TIME_AS_NSECONDS (file->duration),
          file->sequence, file->discont);
    }

    if (file != NULL) {
      if (sscanf (file->uri + 2, g_path_get_basename (sink->location),
              &sink->index) != 1) {;
        GST_ERROR ("Failed to read index from filename: %s", file->uri + 2);
      }
      sink->index++;
    } else {
      sink->index = 0;
    }
    GST_DEBUG_OBJECT (sink, "updated index %ud", sink->index);

    /* If "append-list" option is enabled, the start number of segment file
     * should be started from parsed sequence number from playlist.m3u8.
     */
    g_object_set (sink->multifilesink, "index", sink->index, NULL);
...
gst_hls_sink_handle_message (GstBin * bin, GstMessage * message)
{
  ...
  switch (message->type) {
    case GST_MESSAGE_ELEMENT:
    {
      ...
      // first entry after recovery
      if (sink->append_list &&
          !(sink->state & GST_M3U8_PLAYLIST_RENDER_STARTED) &&
          sink->playlist->entries->length > 0) {
        discont = TRUE;
      }
    ...
    case GST_MESSAGE_EOS:{
      sink->playlist->end_list = !sink->omit_endlist;
      gst_hls_sink_write_playlist (sink);
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_APPEND_LIST:
      ...
      break;
    case PROP_OMIT_ENDLIST:
      sink->omit_endlist = g_value_get_boolean (value);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_APPEND_LIST:
      ...
      break;
    case PROP_OMIT_ENDLIST:
      g_value_set_boolean (value, sink->omit_endlist);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
